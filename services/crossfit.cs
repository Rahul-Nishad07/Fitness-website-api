...
namespace COMMON_PROJECT_STRUCTURE_API.services
{
    public class crossfit
    {
        ...
        public async Task<responseData> Crossfit(requestData rData)
        {
            responseData resData = new responseData();
            try
            {
                var query = @"select * from pc_student.subscribe where id=@id";
                ...
                new MySqlParameter("@fname", rData.addInfo["fname"]) ,
                 new MySqlParameter("@lname",rData.addInfo["lname"]),
                   new MySqlParameter("@email",rData.addInfo["email"]),
                    ...
                    resData.rData["rMessage"] = "Thanks For Subscribe Our Crossfit Classes";
